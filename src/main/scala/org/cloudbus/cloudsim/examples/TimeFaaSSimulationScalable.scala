...
// main-class
object TimeFaaSSimulationScalable extends App with LazyLogging {

  // Lightbend (Typesafe) configuration manager loading .conf files
  val defaultConfig: Config = ConfigFactory.parseResources("defaults_simulation1.conf")
  val fallbackConfig: Config = ConfigFactory.parseResources("overrides_simulation1.conf")
  ...
  try {
    ...
    @SuppressWarnings(Array.apply("unused")) val datacenter0 = TimeFaaSSimulationScalable.createDatacenter("Datacenter_0")
    @SuppressWarnings(Array.apply("unused")) val datacenter1 = TimeFaaSSimulationScalable.createDatacenter("Datacenter_1")
    logger.info("CIS created")
    logger.info("Datacenter creation completed")

    //Third step: Create Broker
    val Some(broker: DatacenterBroker) = TimeFaaSSimulationScalable.createBroker
    ...
    val vmlist = TimeFaaSSimulationScalable.createVM(brokerId, total_vms) //creating 20 vms
    ...
    val cloudletList = TimeFaaSSimulationScalable.createCloudlet(brokerId, total_cloudlets) // creating 40 cloudlets
    ...
    TimeFaaSSimulationScalable.printCloudletList(newList)
