...
// main-class
object SpaceFaaSSimulationScalable extends App with LazyLogging {

  // Lightbend (Typesafe) configuration manager loading .conf files
  val defaultConfig: Config = ConfigFactory.parseResources("defaults_simulation2.conf")
  val fallbackConfig: Config = ConfigFactory.parseResources("overrides_simulation2.conf")
  ...
  try {
    ...
    @SuppressWarnings(Array.apply("unused")) val datacenter0 = SpaceFaaSSimulationScalable.createDatacenter("Datacenter_0")
    @SuppressWarnings(Array.apply("unused")) val datacenter1 = SpaceFaaSSimulationScalable.createDatacenter("Datacenter_1")
    logger.info("CIS created")
    logger.info("Datacenter creation completed")

    //Third step: Create Broker
    val Some(broker: DatacenterBroker) = SpaceFaaSSimulationScalable.createBroker
    ...
    val vmlist = SpaceFaaSSimulationScalable.createVM(brokerId, total_vms) //creating 20 vms
    ...
    val cloudletList = SpaceFaaSSimulationScalable.createCloudlet(brokerId, total_cloudlets) // creating 40 cloudlets
    ...
    SpaceFaaSSimulationScalable.printCloudletList(newList)
